...
ff_node<- read_csv("node_ff.csv") #node table for ff network
names(ff_node) <- c("name","id","party")
ff_node<- ff_node[,c(2,1,3)] #igraph requires specific order for inputs
ff_edge<- read_csv("edge_ff.csv") #edge table for ff network
ff_edge<- ff_edge[,c(2:3,1)]
...
```{r}
rt_node<- read_csv("node_rt.csv") #node table for rt network
names(rt_node) <- c("name","id","party")
rt_node<- rt_node[,c(2,1,3)] #igraph requires specific order for inputs
rt_edge<- read_csv("edge_rt.csv") #edge table for rt network
...
```{r}
mtn_node<- read_csv("node_mtns.csv") #node table for mtn network
names(mtn_node) <- c("name","party","id")
mtn_node<- mtn_node[,c(3,1,2)] #igraph requires specific order for inputs
mtn_edge<- read_csv("edge_mtns.csv") #edge table for mtn network
mtn_edge<- mtn_edge[,1:3]
...
ei_plot<- ggplot(data=ei_df, aes(x=party, y=ei_index, fill=network_type)) +
  ...
  scale_y_continuous(limits=c(-1, 0.5), breaks=c(-1.5,-1,-0.5,0,0.5)) + 
